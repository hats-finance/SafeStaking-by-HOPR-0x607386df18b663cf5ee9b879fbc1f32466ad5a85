...
impl<T: AsyncKVStorage<Key = Box<[u8]>, Value = Box<[u8]>>> HoprCoreEthereumDbActions for CoreEthereumDb<T> {
    ...
    async fn add_to_network_registry(
        ...
        debug!("looking for node {} in network registry", account.to_hex());
        ...
        for pk in public_keys.iter() {
            if public_key == pk {
                let _ = self
                    ...
                    .await?;
                debug!("already found public key {} matching address {}", pk, account.to_hex());
                return Ok(());
            }
        }
        ...
        debug!("performing batch operation");
        let r = self.db.batch(batch_ops, true).await;
        debug!("performing batch operation done");
        r
    }
